...
Every start from a stopped state (play, seek, loop wrap, autoplay, restart at the end and resume after an output-device change) goes through one path that starts all five players on the same frame of their shared render timeline: `play(at:)` receives a sample time about 110 ms ahead of the players' last render time. A host-time start made each `play(at:)` block for about one IO cycle and, measured on a real Mac, still started roughly one seek in five out of sync; with a sample time the calls return immediately and every seek in a polarity null test stayed aligned on macOS 15, 26 and 27. Before the engine has rendered once there is no timeline, so the first start waits up to 100 ms for one and otherwise falls back to a host-time lead sized from the IO buffer and the measured duration of the calls, retrying from the same frame if they overran it.
