...
1. Reserve import state before suspending. Dropped or chosen folders expand to supported audio files off the main actor, in Finder order and without duplicates, and iCloud Drive placeholders in the batch start downloading. Files are processed one at a time.
...
Quitting asks for confirmation while a separation or export runs. Confirming during a separation cancels it and waits up to 15 seconds for its temporary files to be removed; confirming during an export quits at once, and because the exporter stages its output in the volume's item-replacement directory, nothing partial is left in the destination folder.
